...
void read_block_cb(SPIRIT1_CONFIG * sconf, uint8_t addr, uint8_t len, data_callback callback){

  sconf->SPI_start();

  sconf->SPI_transfer(SPIRIT1_SPI_READ);
  sconf->SPI_transfer(addr);

  int i;
  for(i = 0; i < len; i++){
    sconf->SPI_transfer(0x00);
  }

  sconf->SPI_stop(callback);
...
static void readreg8_cb(SPIRIT1_CONFIG * sconf, uint8_t addr, data_callback callback){
...
uint8_t SPIRIT1_rxfifo_dump(SPIRIT1_CONFIG * sconf, uint8_t * data, uint8_t len){
  ...
  uint8_t bytes_to_read = UTIL_MIN(bytes_available, len);
...
uint32_t SPIRIT1_get_irq_status_from_data(uint8_t *data){
    uint32_t val = (data[0] << 24) | (data[1] << 16) | (data[2] << 8) | data[3];
    return val;
}

void SPIRIT1_get_irq_status_cb(SPIRIT1_CONFIG * sconf, data_callback cb){

  read_block_cb(sconf, SPIRIT1_IRQ_STATUS_3, 4, cb);

}

void SPIRIT1_get_rx_fifo_len_cb(SPIRIT1_CONFIG * sconf, data_callback callback){

  readreg8_cb(sconf, SPIRIT1_LINEAR_FIFO_STATUS_0, callback);
...
void SPIRIT1_setup_FIFO_thresholds(SPIRIT1_CONFIG * sconf, uint8_t rx_full, uint8_t rx_empty, uint8_t tx_full, uint8_t tx_empty){

    writereg8(sconf, SPIRIT1_FIFO_CONFIG_0, tx_empty);

}

void SPIRIT1_enable_persistent_tx(SPIRIT1_CONFIG * sconf){
    writereg8(sconf, SPIRIT1_PROTOCOL_0, 0x8 | (1 << SPIRIT1_PERS_TX_SHFIT));
}

void SPIRIT1_disable_persistent_tx(SPIRIT1_CONFIG * sconf){
    writereg8(sconf, SPIRIT1_PROTOCOL_0, 0x8);
}

void SPIRIT1_enable_tx(SPIRIT1_CONFIG * sconf){
    command(sconf, SPIRIT1_CMD_TX);
}

void SPIRIT1_goto_ready(SPIRIT1_CONFIG * sconf){
    command(sconf, SPIRIT1_CMD_READY);
}

void SPIRIT1_abort(SPIRIT1_CONFIG * sconf){
    command(sconf, SPIRIT1_CMD_SABORT);
}
